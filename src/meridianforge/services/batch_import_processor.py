...
class BatchImportProcessor:
    ...
    def process(
        ...
        processed = len(pipeline_result.assets)

        failed = len(records) - processed

        confidence = BatchConfidence.calculate(
            [pipeline_result.confidence],
            failed_records=failed,
            total_records=len(records),
            unknown_fields=len(pipeline_result.warnings),
        )
        ...
        return BatchImportResult(
            ...
            warnings=[str(item) for item in pipeline_result.warnings],
